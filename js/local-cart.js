...
class LocalCart {
    ...
    dispatchCartUpdate() {
        window.dispatchEvent(
            new CustomEvent('cartUpdated', { detail: this.cart })
        );
    }
    ...
    addItem(product) {
        ...
        if (!product.variantId) {
            console.error(
                'LocalCart: addItem отримав товар без variantId',
                product
            );
        ...
        const normalizeOptions = (options) => {
            ...
            if (options.packageVariant)
                normalized.packageVariant = String(
                    options.packageVariant
                ).trim();
        ...
        const existingIndex = this.cart.findIndex((item) => {
            ...
            return productKeys.every(
                (key) => productOptions[key] === itemOptions[key]
            );
        });
    ...
    isProductPage() {
        ...
        const pageTypeElements = document.querySelectorAll(
            '[data-page-type], [style*="pageType"]'
        );
        for (const el of pageTypeElements) {
            const pageType =
                el.getAttribute('data-page-type') ||
                (el.getAttribute('style')?.includes('pageType:GP_PRODUCT')
                    ? 'GP_PRODUCT'
                    : null);
    ...
    init() {
        ...
        document.addEventListener(
            ...
            (e) => {
                ...
                const button = e.target.closest(
                    'button[type="submit"][name="add"], button[type="submit"].gp-button-atc, button.gp-button-atc, button[data-add-to-cart], .add-to-cart-button'
                );
        ...
        setTimeout(() => {
            if (this.isProductPage()) {
                ...
                const gpProductButtons =
                    document.querySelectorAll('gp-product-button');
    ...
    handleAddToCart(form) {
        try {
                ...
                try {
                    if (gpContext) {
                        ...
                        const decodedContext =
                            this.decodeHtmlEntities(gpContext);
                        const context = JSON.parse(decodedContext);
                        if (
                            context.variantSelected &&
                            context.variantSelected.id
                        ) {
                    ...
                    console.warn(
                        'LocalCart: Помилка парсингу gp-context/gp-data для variantId',
                        e,
                        {
            ...
            const allOptionSelects = form.querySelectorAll(
                'select[name^="options"]'
            );
            const allOptionRadios = form.querySelectorAll(
            ...
            for (const select of allOptionSelects) {
                if (
                    ...
                    const label =
                        form.querySelector(`label[for="${select.id}"]`) ||
                    ...
                    this.showNotification(
                        `Будь ласка, оберіть ${optionName || 'опцію'}`
                    );
                    return;
                }
            }
            ...
            for (const groupName of radioGroups) {
                ...
                if (!checked) {
                    ...
                    const label = firstRadio
                        ? firstRadio.closest('label') ||
                          form.querySelector(`label[for="${firstRadio.id}"]`) ||
                          firstRadio
                              .closest('.gp-variant-selector')
                              ?.querySelector('label') ||
                          firstRadio.closest('[data-option-name]')
                        : null;
                    const optionName = label
                        ? label.textContent?.trim() ||
                          label.getAttribute('data-option-name') ||
                          'опцію'
                        : 'опцію';
                    this.showNotification(
                        `Будь ласка, оберіть ${optionName || 'опцію'}`
            ...
            if (!productData) {
                console.error(
                    'LocalCart: extractProductData повернув null/undefined'
                );
                this.showNotification(
                    'Помилка: не вдалося отримати дані товару'
                );
                return;
            }
            ...
            if (
                ...
                console.error(
                    'LocalCart: Не вдалося отримати назву товару',
                    productData
                );
                this.showNotification(
                    'Помилка: не вдалося отримати назву товару'
                );
                return;
            }
            ...
            if (
                ...
                console.warn(
                    'LocalCart: Ціна не знайдена, встановлюємо 0',
                    productData
                );
                productData.price = 0;
            }
            ...
            if (variant1) {
                const svg1 = variant1.querySelector(
                    'svg circle[fill="#00237E"]'
                );
                if (
                    svg1 &&
                    svg1
                        .closest('svg')
                        .querySelector('circle[fill="#00237E"]:not([stroke])')
                ) {
            ...
            if (!packageVariant && variant2) {
                const svg2 = variant2.querySelector(
                    'svg circle[fill="#00237E"]'
                );
                if (
                    svg2 &&
            ...
            if (!packageVariant) {
                const buy5Option =
                    selectedOptions['Buy 5 & Get 5 FREE'] ||
                    selectedOptions.option1;
                if (buy5Option) {
                    if (
                        buy5Option.includes(
                            '2x Black + 1x Blue + 1x Red + 1x Grey'
                        ) ||
                        ...
                        buy5Option.includes(
                            '4x Black + 2x Red + 2x Blue + 2x Grey'
                        ) ||
            ...
            let colorInfo = '';
            const colorOption =
                selectedOptions.Color || selectedOptions.option3;
            if (colorOption) {
            ...
            if (titleParts.length > 0) {
                ...
                if (selectedOptions.public_title) {
                    ...
                    const optionValues = Object.values(selectedOptions).filter(
                        (v) => v && v !== '' && v !== 'null'
                    );
            ...
            if (
                ...
                (typeof finalImage === 'string' &&
                    finalImage.includes('undefined'))
            ) {
                console.warn(
                    'LocalCart: Проблема з зображенням, використовуємо fallback',
                    {
                        finalImage,
                        packageVariant,
            ...
            if (
                ...
                finalImage === '' ||
                finalImage === 'undefined' ||
                finalImage === productData.image + 'undefined'
            ) {
                console.warn(
                    'LocalCart: Проблема з зображенням, використовуємо оригінальне',
                    {
                        finalImage,
                        productDataImage: productData.image,
                        packageVariant,
    ...
    extractProductData(form, variantIdOverride = null) {
                    ...
                    if (product) {
                        productInfo = {
                            ...
                            price: product.offers?.price
                                ? parseFloat(product.offers.price) * 100
                                : productInfo?.price,
                            image:
                                product.image?.[0] ||
                                product.image ||
        ...
        try {
            ...
            for (const script of allScripts) {
                ...
                if (
                    content.includes('var productInfo') ||
                    ...
                    for (const pattern of patterns) {
                        ...
                        if (match) {
                            try {
                                ...
                                const gpContext =
                                    document.querySelector('[gp-context]');
                                if (gpContext) {
                                    try {
                                        const gpContextRaw =
                                            gpContext.getAttribute(
                                                'gp-context'
                                            );
                                        const gpContextDecoded =
                                            this.decodeHtmlEntities(
                                                gpContextRaw
                                            );
                                        const gpData =
                                            JSON.parse(gpContextDecoded);
                                        if (gpData.productId) {
        ...
        const priceElement = document.querySelector(
            '#p-price-gcm86FEEK0 .gp-price.gp-money'
        );
        if (priceElement) {
            ...
            if (priceMatch) {
                ...
                if (!isNaN(priceValue) && priceValue > 0) {
                    price = priceValue;
                    console.log(
                        'LocalCart: Ціна витягнута з елемента сторінки:',
                        price
                    );
                }
            }
        ...
        if (!price || price === 0) {
            const anyPriceElement = document.querySelector(
                '.gp-price.gp-money:not(.gp-product-compare-price)'
            );
            if (anyPriceElement) {
                ...
                if (priceMatch) {
                    ...
                    if (!isNaN(priceValue) && priceValue > 0) {
                        price = priceValue;
                        console.log(
                            'LocalCart: Ціна витягнута з будь-якого елемента gp-price:',
                            price
                        );
                    }
                }
            ...
            if (variant) {
                ...
                if (variant.price) {
                    ...
                    if (variantPrice < 100) {
                        price = variantPrice * 45;
                        console.log(
                            'LocalCart: Ціна з variant конвертована з доларів:',
                            price
                        );
                    } else {
                        price = variantPrice;
                    ...
                    const productPrice =
                        typeof productInfo.price === 'number'
                            ...
                            : parseFloat(productInfo.price) / 100;
                    if (productPrice < 100) {
                        price = productPrice * 45;
                        console.log(
                            'LocalCart: Ціна з productInfo конвертована з доларів:',
                            price
                        );
                    } else {
                        price = productPrice;
            ...
            if (!variant && (!price || price === 0)) {
                if (productInfo.price) {
                    ...
                    if (productPrice < 100) {
                        price = productPrice * 45;
                        console.log(
                            'LocalCart: Ціна з productInfo (без variant) конвертована з доларів:',
                            price
                        );
                    } else {
                        price = productPrice;
        ...
        if ((!price || price === 0) && ogPrice) {
            ...
            if (ogPriceValue < 100) {
                ...
                console.log(
                    'LocalCart: Ціна з og:price конвертована з доларів:',
                    price
                );
            } else {
                price = ogPriceValue;
        ...
        if (!title || title === 'undefined' || title === '') {
            console.warn(
                'LocalCart: Назва товару не знайдена, використовуємо fallback'
            );
            title = 'Товар';
        }
    ...
    getSelectedOptions(form) {
            ...
            try {
                if (gpContext) {
                    ...
                    if (context.variantSelected) {
                        if (context.variantSelected.option1)
                            options['option1'] =
                                context.variantSelected.option1;
                        if (context.variantSelected.option2)
                            options['option2'] =
                                context.variantSelected.option2;
                        if (context.variantSelected.option3)
                            options['option3'] =
                                context.variantSelected.option3;
                        if (context.variantSelected.public_title)
                            options['public_title'] =
                                context.variantSelected.public_title;
                    }
                }
                ...
                if (gpData && Object.keys(options).length === 0) {
                    ...
                    if (data.variantSelected) {
                        if (data.variantSelected.option1)
                            options['option1'] = data.variantSelected.option1;
                        if (data.variantSelected.option2)
                            options['option2'] = data.variantSelected.option2;
                        if (data.variantSelected.option3)
                            options['option3'] = data.variantSelected.option3;
                        if (data.variantSelected.public_title)
                            options['public_title'] =
                                data.variantSelected.public_title;
                    }
                }
            } catch (e) {
                console.warn(
                    'LocalCart: Помилка парсингу gp-context/gp-data для опцій',
                    e
                );
            }
        }
        ...
        variantSelectors.forEach((variantSelector) => {
            ...
            const buy5Option = variantSelector.querySelector(
                'div[option-name="Buy 5 & Get 5 FREE"], div[variant-option-name="Buy 5 & Get 5 FREE"]'
            );
            if (buy5Option) {
                const checked = buy5Option.querySelector(
                    'input[type="radio"]:checked, label[data-selected="true"], label:has(input:checked)'
                );
                if (checked) {
                    const optionValue =
                        checked.getAttribute('option-data') ||
                        checked
                            .querySelector('.option-value-wrapper')
                            ?.getAttribute('option-data') ||
                        checked
                            .querySelector('[option-data]')
                            ?.getAttribute('option-data') ||
            ...
            // Шукаємо опцію "Size"
            const sizeOption = variantSelector.querySelector(
                'div[option-name="Size"], div[variant-option-name="Size"]'
            );
            if (sizeOption) {
                const checked = sizeOption.querySelector(
                    'input[type="radio"]:checked, select option:checked, [data-selected="true"], label:has(input:checked)'
                );
                if (checked) {
                    const optionValue =
                        checked.value ||
        ...
        if (Object.keys(options).length === 0) {
            const variantSelectors = form.querySelectorAll(
                '.gp-variant-selector, [data-option-name]'
            );
            variantSelectors.forEach((selector) => {
                const checked = selector.querySelector(
                    'input[type="radio"]:checked, select option:checked, [data-selected="true"]'
                );
                if (checked) {
                    const optionName =
    ...
    watchVariantChanges() {
        ...
        const variantSelectors = document.querySelectorAll(
            'select[name^="options"], input[type="radio"][name^="options"]'
        );
                ...
                if (form) {
                    ...
                    if (gpProduct) {
                        ...
                        try {
                            ...
                            console.warn(
                                'LocalCart: Помилка оновлення variantId',
                                e
                            );
                        }
                    }
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (
                    ...
                    mutation.attributeName === 'gp-context'
                ) {
                    const gpProduct = mutation.target;
                    if (gpProduct.tagName === 'GP-PRODUCT') {
                        const form = gpProduct.querySelector(
                            'form[action*="/cart/add"]'
                        );
                        if (form) {
                            try {
                                const gpContext =
                                    gpProduct.getAttribute('gp-context');
                                const context = JSON.parse(gpContext);
                                if (
                                    context.variantSelected &&
                                    context.variantSelected.id
                                ...
                                console.warn(
                                    'LocalCart: Помилка оновлення variantId через observer',
                                    e
                                );
                            }
                        }
        ...
        gpProducts.forEach((gpProduct) => {
            observer.observe(gpProduct, {
                attributes: true,
                attributeFilter: ['gp-context', 'gp-data'],
            });
        });
    }
    ...
    convertHomepageFormsToLinks() {
        ...
        productForms.forEach((productForm) => {
            ...
            try {
                ...
                console.warn(
                    'LocalCart: Помилка парсингу gp-data/gp-context',
                    e
                );
            }
            ...
            if (productUrl) {
                ...
                const buttons = form.querySelectorAll(
                    'button[type="submit"], .gp-content-product-button, [data-add-to-cart]'
                );
    ...
    makeProductImagesClickable(gpProduct, productUrl) {
        ...
        const imageContainers = gpProduct.querySelectorAll(
            '.gp-group/image:not(a .gp-group/image), picture:not(a picture)'
        );
    ...
    handleCheckout(formModal) {
        const formData = new FormData(
            formModal.querySelector('#checkout-form')
        );
        const orderData = {
        ...
        const orders = JSON.parse(
            localStorage.getItem('boxhero_orders') || '[]'
        );
